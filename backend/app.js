...
const allowedOrigins = process.env.NODE_ENV === 'production' 
  ...
  : ['http://localhost:3000', 'https://stitch.withgoogle.com'];
=======
...
const app = express();
const PORT = process.env.PORT || 3000;
>>>>>>> 9bb6e534fcc749e2ce0794be12d63b0253b9d50e
...
<<<<<<< HEAD
  const { data, error } = await supabase
    .from('meetings')
    .insert([{ title, user_id: req.user.sub, status: 'DRAFT' }])
    .select().single();
...
v1.post('/meetings/:id/audio:prepare-upload', verifyToken, async (req, res) => {
  const storagePath = `audios/${req.user.sub}/${req.params.id}.webm`;
